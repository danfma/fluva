import { AbstractValidator, equalTo } from "@src"
...
describe("EqualTo", () => {
  ...
  async function expectInvalid(request: ChangePasswordRequest) {
  ...
  async function expectValid(request: ChangePasswordRequest) {
    const result = await validator.validate(request)
